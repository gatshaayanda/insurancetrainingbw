...
export default function AdminHubLoader() {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex flex-col items-center justify-center transition-opacity duration-700 bg-gradient-to-br from-[#3A8DDB] to-[#0F264B] ${
        fading ? 'opacity-0' : 'opacity-100'
      }`}
    >
      <div className="relative flex items-center justify-center mb-6">
        <div className="animate-spin rounded-full h-20 w-20 border-t-4 border-b-4 border-white/40"></div>
        <span className="absolute text-3xl font-extrabold text-white tracking-widest">ITBW</span>
      </div>
      <span className="text-white font-medium animate-pulse text-sm tracking-wide">
        Loading Insurance Training BW...
      </span>
    </div>
  );
}
